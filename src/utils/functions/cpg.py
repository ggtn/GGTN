...
def parse_to_nodes_cfg(cpg, max_nodes=500):
    ...
    for function in cpg["functions"]:
        ...
        for node in filtered_ast_nodes:
            if node in filtered_cfg_nodes:
                filtered_ast_nodes[node]=filtered_cfg_nodes[node]
            else:
                filtered_ast_nodes[node].drop_edges()

        cfg_nodes.update(filtered_ast_nodes)
...
def parse_to_nodes_pdg(cpg, max_nodes=500):
    ...
    for function in cpg["functions"]:
        ...
        for node in filtered_ast_nodes:
            if node in filtered_pdg_nodes:
                filtered_ast_nodes[node] = filtered_pdg_nodes[node]
            else:
                filtered_ast_nodes[node].drop_edges()

        pdg_nodes.update(filtered_ast_nodes)
